...
Al usar runners auto-hospedados, las máquinas virtuales `odoo-server` y `db-server` pueden recibir órdenes y código directamente desde GitHub para actualizarse automáticamente, evitando la necesidad de exponer SSH al exterior.
...
- **Ubicación:** `odoo-server` (Debian 13, IP `192.168.30.10`, VLAN 30 DMZ)
...
- **Ubicación:** `db-server` (Debian 13, IP `192.168.40.10`, VLAN 40 Administración)
...
- `odoo-server` se enruta a través de `192.168.30.1` (pfSense VLAN 30).
- `db-server` se enruta a través de `192.168.40.1` (pfSense VLAN 40).
